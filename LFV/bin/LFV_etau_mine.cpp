...
int main(int argc, char* argv[]) {
 ...
 vector<string> bkg_procs = {"ZTT", "ZL", "VV", "TT", "Mis", "SMH_gg", "SMH_qq"};//VV
 ...
 cb.cp().process(ch::JoinStr({sig_procs, {"ZTT", "TT", "ZL", "SMH_gg", "SMH_qq","VV"}}))//VV
  .AddSyst(cb, "CMS_eff_e", "lnN", SystMap<>::init(1.02));

 cb.cp().process(ch::JoinStr({sig_procs, {"ZTT", "TT", "SMH_gg", "SMH_qq", "VV"}}))//VV
  .AddSyst(cb, "CMS_eff_tau", "lnN", SystMap<>::init(1.05));

 cb.cp().process(ch::JoinStr({sig_procs, {"ZTT", "TT", "ZL", "VV", "SMH_gg", "SMH_qq"}}))//VV
  .AddSyst(cb, "lumi_13TeV", "lnN", SystMap<>::init(1.026));

 cb.cp().process({"LFV_gg125","SMH_gg"})
  .AddSyst(cb, "pdf_gg", "lnN", SystMap<>::init(1.1));

 cb.cp().process({"LFV_vbf125","SMH_qq"})
 ...
 cb.cp().process({"ZL"})
  .AddSyst(cb, "norm_dyother", "lnN", SystMap<>::init(1.25));
 ...
 cb.cp().process(ch::JoinStr({sig_procs, {"ZTT", "ZL", "TT", "SMH_gg", "SMH_qq", "VV"}}))//VV
  .AddSyst(cb, "CMS_scale_e_13TeV", "shape", SystMap<>::init(1.00));

 cb.cp().process(ch::JoinStr({sig_procs, {"ZTT", "TT", "SMH_gg", "SMH_qq", "VV"}}))
  .AddSyst(cb, "CMS_scale_t_13TeV", "shape", SystMap<>::init(1.00));
 ...
 cb.cp().process(ch::JoinStr({sig_procs, {"ZTT", "ZL", "TT", "SMH_gg", "SMH_qq", "VV"}}))
...
// Added 27 JES
 ...
 //
  ...
  // Adding the avg shift anyway, for the MET
  else cb.cp().process(ch::JoinStr({sig_procs, {"ZTT", "ZL", "TT", "SMH_gg", "SMH_qq", "VV"}})).AddSyst(cb, "CMS_scale_j_13TeV", "shape", SystMap<>::init(1.00));
